...
import "https://cdnjs.cloudflare.com/ajax/libs/jspdf-autotable/3.8.2/jspdf.plugin.autotable.min.js";

// ... (kode import Firebase sama seperti sebelumnya) ...
import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";
import { getAuth, onAuthStateChanged, signOut } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js";
import { getFirestore, collection, query, where, orderBy, limit, onSnapshot, doc, getDoc, getDocs, addDoc, updateDoc, deleteDoc, writeBatch, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
function initializeAppLogic(user) {
    ...
    document.getElementById('main-nav').addEventListener('click', (e) => {
        ...
        if (link && !link.classList.contains('btn-logout')) { // Pastikan link logout tidak memicu navigasi
             e.preventDefault(); 
    ...
    function navigate(page) {
        ...
        else if (page === 'buat-report') loadBuatReportPage();
        else loadDataTablePage();
    }

    // --- Template & Form Builders ---
    async function loadDashboardPage() { /* ... kode sama, tapi log dihapus ... */ 
    ...
    // FUNGSI BARU: Untuk halaman "Keluar Masuk"
    function loadKeluarMasukPage() {
        contentArea.innerHTML = `
            ...
            <div class="card"><div class="card-header d-flex justify-content-between align-items-center">
                ...
                <button class="btn btn-outline-secondary btn-sm" id="clear-log-btn"><i class="bi bi-trash me-1"></i> Bersihkan Semua Log</button>
            </div><div class="card-body p-0 scrollable-container" id="log-container"><ul class="list-group list-group-flush"></ul></div></div>`;
        renderLogs(true); // Parameter true untuk menampilkan semua log
        document.getElementById('clear-log-btn').addEventListener('click', () => { /* ... logika clear log ... */ });
    }

    // FUNGSI BARU: Untuk halaman "Buat Report"
    function loadBuatReportPage() {
        contentArea.innerHTML = `
            ...
            <div class="card">
                <div class="card-header">Konfigurasi Laporan PDF</div>
                <div class="card-body">
                    <form id="reportForm">
                        <h6>1. Pilih Data yang Akan Dimasukkan</h6>
                        <div class="form-check"><input class="form-check-input" type="checkbox" value="material" id="checkMaterial" checked><label class="form-check-label" for="checkMaterial">Data Material</label></div>
                        <div class="form-check"><input class="form-check-input" type="checkbox" value="komponen" id="checkKomponen" checked><label class="form-check-label" for="checkKomponen">Data Komponen</label></div>
                        <div class="form-check"><input class="form-check-input" type="checkbox" value="keluar" id="checkKeluar" checked><label class="form-check-label" for="checkKeluar">Barang Keluar</label></div>
                        <div class="form-check"><input class="form-check-input" type="checkbox" value="logs" id="checkLogs"><label class="form-check-label" for="checkLogs">Log Aktivitas</label></div>
                        
                        <h6 class="mt-4">2. Pilih Rentang Waktu (Untuk Log)</h6>
                        <div class="row">
                            <div class="col-md-6 mb-3"><label for="startDate" class="form-label">Tanggal Mulai</label><input type="date" id="startDate" class="form-control"></div>
                            <div class="col-md-6 mb-3"><label for="endDate" class="form-label">Tanggal Akhir</label><input type="date" id="endDate" class="form-control"></div>
                        </div>

                        <button type="submit" class="btn btn-primary w-100 mt-3"><i class="bi bi-download me-2"></i>Download Report (PDF)</button>
                    </form>
                </div>
            </div>`;

        document.getElementById('reportForm').addEventListener('submit', generatePDFReport);
    }
    
    // FUNGSI BARU: Untuk men-generate PDF
    async function generatePDFReport(e) {
        ...
        doc.setFontSize(18);
        doc.text("Laporan Inventaris", 14, yPos);
        yPos += 10;
        doc.setFontSize(10);
        doc.text(`Dibuat pada: ${new Date().toLocaleString('id-ID')}`, 14, yPos);
        yPos += 15;

        const dataToInclude = Array.from(document.querySelectorAll('#reportForm input[type=checkbox]:checked')).map(el => el.value);

        for (const collectionName of dataToInclude) {
            if (collectionName === 'logs') continue; // Skip logs for now, handle later

            const snapshot = await getDocs(collection(db, 'users', user.uid, collectionName));
            if (!snapshot.empty) {
                const config = {
                    'material': { title: 'Data Material', headers: ['NAMA MATERIAL', 'MERK', 'SPESIFIKASI', 'JUMLAH', 'KETERANGAN']},
                    'komponen': { title: 'Data Komponen', headers: ['NAMA KOMPONEN', 'MERK', 'SPESIFIKASI', 'JUMLAH', 'KETERANGAN']},
                ...
                const head = [pageConfig.headers];
                const body = snapshot.docs.map(doc => {
                    const data = doc.data();
                    const keys = Object.keys(config[collectionName].form); // Need to define form keys in config
                    return keys.map(key => data[key] || '');
                });
                
                doc.setFontSize(14);
                doc.text(pageConfig.title, 14, yPos);
                yPos += 8;
        ...
        // ... (Logika untuk menambahkan logs ke PDF) ...

        doc.save(`report-inventaris-${Date.now()}.pdf`);
    }

    // --- Modifikasi Page Loader & Renderers ---
    function loadDataTablePage() {
        ...
        contentArea.innerHTML = `
            ...
            <div class="card mb-4"><div class="card-header bg-white d-flex justify-content-between align-items-center"><h5>Daftar ${pageConfig.title}</h5><button class="btn btn-primary btn-tambah"><i class="bi bi-plus-circle me-2"></i>Tambah ${pageConfig.title}</button></div><div class="card-body p-0"><div class="table-responsive scrollable-container"><table class="table table-striped table-hover table-bordered mb-0"><thead class="table-dark"><tr><th>NO</th>${pageConfig.headers.map(h => `<th>${h}</th>`).join('')}<th>OPSI</th></tr></thead><tbody id="table-body"></tbody></table></div></div></div>
            <div class="card"><div class="card-header d-flex justify-content-between align-items-center">Riwayat Aktivitas ${pageConfig.title}</div><div class="card-body p-0 scrollable-container" id="log-container"><ul class="list-group list-group-flush"></ul></div></div>`;
        renderTableData(pageConfig.headers);
        renderLogs();
    }

    function renderLogs(showAll = false) {
        const logContainer = document.querySelector('#log-container ul');
        ...
        onSnapshot(q, (snapshot) => {
            const logs = showAll ? snapshot.docs : snapshot.docs.filter(doc => doc.data().collection === currentCollectionName);
            if (logs.length === 0) { logContainer.innerHTML = '<li class="list-group-item text-muted text-center">Belum ada aktivitas.</li>'; return; }
            let logHtml = '';
            logs.forEach(doc => {
                const log = doc.data();
                const details = log.details || {};
                ...
                const itemDetails = ` (Merk: ${details.merk || '-'}, Spek: ${details.spesifikasi || '-'}, Jml: ${details.jumlah || 0}, Ket: ${details.keterangan || '-'})`;
                const date = log.timestamp?.toDate().toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' }) || 'Baru saja';
                logHtml += `<li class="list-group-item">[${date}] Item <strong>${docName}</strong> telah <strong>${log.action}</strong> oleh ${log.user}.${itemDetails}</li>`;
            });
            logContainer.innerHTML = logHtml;
        });
    }
